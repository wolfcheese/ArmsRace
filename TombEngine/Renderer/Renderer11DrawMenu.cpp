...
namespace TEN::Renderer
{
	...
	void Renderer11::RenderOptionsMenu(Menu menu, int initialY)
	{
		...
		case Menu::Options:
			...
			// Language
			AddString(MenuCenterEntry, y, g_GameFlow->GetString("language_option"), PRINTSTRING_COLOR_WHITE, SF_Center(title_option == 3));
			break;
		...
		case Menu::OtherSettings:
			...
			//wc -  Fast Load
			AddString(MenuLeftSideEntry - 100, y, g_GameFlow->GetString("fastLoad"), PRINTSTRING_COLOR_ORANGE, SF(title_option == 6));
			AddString(MenuRightSideEntry, y, Str_Enabled(g_Gui.GetCurrentSettings().conf.fastLoad > 0), PRINTSTRING_COLOR_WHITE, SF(title_option == 6));
			GetNextBlockPosition(&y);
		...
		case Menu::Language:
			...
			AddString(MenuCenterEntry, y, g_GameFlow->GetString("language"), PRINTSTRING_COLOR_YELLOW, SF_Center());

			GetNextLinePosition(&y);

			AddString(MenuLeftSideEntry, y, g_GameFlow->GetString("english"), g_Gui.GetCurrentSettings().conf.language == 0 ? PRINTSTRING_COLOR_WHITE : PRINTSTRING_COLOR_ORANGE, SF(g_Gui.GetCurrentSettings().conf.language == 0 && title_option == 0));
			AddString(MenuRightSideEntry, y, g_GameFlow->GetString("french"), g_Gui.GetCurrentSettings().conf.language != 0 ? PRINTSTRING_COLOR_WHITE : PRINTSTRING_COLOR_ORANGE, SF(g_Gui.GetCurrentSettings().conf.language != 0 && title_option == 0));

			GetNextLinePosition(&y);

			// Apply
			AddString(MenuCenterEntry, y, g_GameFlow->GetString(STRING_APPLY), PRINTSTRING_COLOR_ORANGE, SF_Center(title_option == 1));
			GetNextLinePosition(&y);

			// Cancel
			AddString(MenuCenterEntry, y, g_GameFlow->GetString(STRING_CANCEL), PRINTSTRING_COLOR_ORANGE, SF_Center(title_option == 2));
			break;
		}
	}
	...
	void Renderer11::RenderGameOverMenu()
	{
		int y = MenuVerticalOptionsPause;
		auto pause_option = g_Gui.GetSelectedOption();

		AddString(MenuCenterEntry, MenuVerticalNarrowLineSpacing, g_GameFlow->GetString("game_over"),
			PRINTSTRING_COLOR_ORANGE, SF_Center());

		GetNextLinePosition(&y);

		// Options
		AddString(MenuCenterEntry, y, g_GameFlow->GetString(STRING_LOAD_GAME), PRINTSTRING_COLOR_WHITE, SF_Center(pause_option == 0));
		GetNextLinePosition(&y);

		// Exit to title
		AddString(MenuCenterEntry, y, g_GameFlow->GetString(STRING_EXIT_TO_TITLE), PRINTSTRING_COLOR_WHITE, SF_Center(pause_option == 1));
	...
	void Renderer11::DrawStatistics()
	{
		...
		// Secrets found
		sprintf(buffer, "%d / 5", Statistics.Game.Secrets);
		AddString(MenuRightSideEntry, y, buffer, PRINTSTRING_COLOR_WHITE, SF());
		...
		float scale1)
	{
		...
		for (int n = 0; n < (*moveableObj).ObjectMeshes.size(); n++)
		{
			...
			//wc - hack to rotate compass needle
			if (objectNum == ID_COMPASS_ITEM && n == 1)
			{
				(*moveableObj).LinearizedBones[n]->ExtraRotation.y = TO_RAD((LaraItem->Pose.Orientation.y) - ANGLE(180));
			}
