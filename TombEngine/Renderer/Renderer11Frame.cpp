...
namespace TEN::Renderer
{
	...
	void Renderer11::CollectRooms(RenderView &renderView, bool onlyRooms)
	{
		...
		//wc - hack to not show extra rooms outside intro area
		bool baseOutsideIntro = baseRoomIndex >= 166 && baseRoomIndex <= 167;
		//wc - hack to avoid clipping issues across outdoor area
		bool baseOutside = !baseOutsideIntro && baseRoomIndex == 115;
		for (int i = 0; i < g_Level.Rooms.size(); i++)
		{
			bool outsideIntro = (i >= 164 && i <= 169) || i == 84;
			bool outside = baseOutside && (i == 18 || i == 14 || i == 69 || i == 70 || i == 4 || i == 26 
				|| i == 27 || i == 28 || i == 10 || i == 8 || i == 3);
			...
			if (baseOutsideIntro && !outsideIntro)
				m_rooms[i].Visited = true;
			else if (outside)
			{
				m_rooms[i].Visited = true;
				renderView.roomsToDraw.push_back(&m_rooms[i]);
	...
	void Renderer11::CollectStatics(short roomNumber, RenderView& renderView)
	{
		...
		for (int i = 0; i < numStatics; i++)
		{
			...
			auto staticInfo = RendererStatic
			{
				...
				world,
				Vector4(0.5f,0.5f,0.5f,1.0f), //wc - static lighting is set to all black, possibly due to my fast loading? just going to hardcode 128/128/128 here
				room.AmbientLight,
				lights
			};
	...
	void Renderer11::CollectLights(Vector3 position, float radius, int roomNumber, int prevRoomNumber, bool prioritizeShadowLight, std::vector<RendererLight*>& lights)
	{
		...
		for (auto& light : dynamicLights)
		{
			...
			if (tempLights.size() == MAX_LIGHTS_PER_ITEM * 4)
				break;
		}
		...
		{
			//wc - only check current room
			if (roomToCheck != roomNumber)
				continue;
