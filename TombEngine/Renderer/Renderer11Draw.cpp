...
//wc - custom bars
extern TEN::Renderer::RendererHUDBar* g_FreezeBar;
extern TEN::Renderer::RendererHUDBar* g_AntiGravBar;
extern TEN::Renderer::RendererHUDBar* g_SFXVolumeBar;
extern TEN::Renderer::RendererHUDBar* g_MusicVolumeBar;
extern TEN::Renderer::RendererHUDBar* g_BaddyBar;
...
namespace TEN::Renderer
{
	...
	{
		//wc - lara double doesn't cast shadow
	...
	void Renderer11::RenderScene(ID3D11RenderTargetView* target, ID3D11DepthStencilView* depthTarget, RenderView& view)
	{
		...
		//wc - display ammo counter
		if (Lara.Control.HandStatus == HandStatus::WeaponReady)
		{
			LaraInfo*& info = LaraItem->Data;
			auto ammo = GetAmmo(LaraItem, Lara.Control.Weapon.GunType);
			auto stringKey = ammo.hasInfinite() ? "unlimited_ammo" : "";
			auto color = PRINTSTRING_COLOR_WHITE;
			if (WillRestoreAmmo())
			{
				if (Lara.Control.Weapon.GunType == LaraWeaponType::Revolver)
				{
					stringKey = "teleport_ammo";
					color = D3DCOLOR_ARGB(255, 153, 7, 186);
				}
				else if (Lara.Control.Weapon.GunType == LaraWeaponType::Shotgun)
				{
					stringKey = "stop_ammo";
					color = D3DCOLOR_ARGB(255, 2, 37, 179);
				}
				else if (Lara.Control.Weapon.GunType == LaraWeaponType::RocketLauncher)
				{
					stringKey = "antigrav_ammo";
					color = D3DCOLOR_ARGB(255, 28, 242, 24);
				}
				else if (Lara.Control.Weapon.GunType == LaraWeaponType::Uzi)
				{
					stringKey = "ice_ammo";
					color = D3DCOLOR_ARGB(255, 7, 215, 247);
				}
				else if (Lara.Control.Weapon.GunType == LaraWeaponType::Crossbow)
				{
					stringKey = "flame_ammo";
					color = D3DCOLOR_ARGB(255, 242, 161, 22);
				}
			}
			std::string tmp = stringKey == "" ?  std::to_string(ammo.getCount()) : g_GameFlow->GetString(stringKey);
			tmp.insert(0, 13 - tmp.size(), ' ');
			const char *chars = tmp.c_str();
			AddString(600, 550, chars, color, 0);
		}
	...
	void Renderer11::UpdateLogo(const wchar_t* path)
	{
		m_logo = std::filesystem::exists(path) ? Texture2D(m_device.Get(), path) : Texture2D();
	}
	...
	void Renderer11::DrawItems(RenderView& view, bool transparent)
	{
		...
		//wc - tell lara doubles which ones are being drawn or not
		...
		for (auto room : view.roomsToDraw)
		{
			for (auto itemToDraw : room->ItemsToDraw)
			{
				...
				case ID_DARTS:
					...
					//wc - hidden obj used for weapon powers
				case ID_LARA_DOUBLE:
				{
					ItemInfo* item = &g_Level.Items[itemToDraw->ItemNumber];
					item->ItemFlags[4] = 1;
					break;
				}
				case ID_WATERFALL1:
				...
				case ID_UZI_AMMO_ITEM:
				{
					...
					for (int i = 0; i < item->Animation.Mutator.size(); i++)
						item->Animation.Mutator[i].Scale = Vector3(1.5f, 1.5f, 1.5f);
				...
				case ID_REVOLVER_AMMO_ITEM:
				{
					ItemInfo* item = &g_Level.Items[itemToDraw->ItemNumber];
					for (int i = 0; i < item->Animation.Mutator.size(); i++)
						item->Animation.Mutator[i].Scale = Vector3(3.0f, 3.0f, 3.0f);
					DrawAnimatingItem(itemToDraw, view, transparent);
					break;
				}
				//wc - super hack - don't draw square pushable blocks from outside rooms (helps with embedded texture issue)
				case ID_PUSHABLE_OBJECT5:
	...
	void Renderer11::DrawStatics(RenderView& view, bool transparent)
	{
		...
		for (auto room : view.roomsToDraw)
		{
			for (auto& msh : room->StaticsToDraw)
			{
				...
				//wc - hack to prevent stairs and bookcase object from appearing in outside wall
				if (staticObj.Id == 104 || staticObj.Id == 109)
					if (g_Level.Rooms[LaraItem->RoomNumber].flags & ENV_FLAG_WIND)
						continue;
