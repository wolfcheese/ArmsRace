...
namespace TEN::Entities::TR4
{
	...
	void SasControl(short itemNumber)
	{
		...
		if (item->HitPoints > 0)
		{
			if (item->TriggerFlags == 999 && BaddyHealthItemNumber <= 0 && item == Lara.TargetEntity)
			{
			...
			else if (item->TriggerFlags == 999 && !creature->Enemy)
				creature->Enemy = LaraItem;

			//wc - 999 teleporting boss, scan for lara_doubles to target
			if (item->TriggerFlags == 999 && ++item->ItemFlags[7] >= 60 && item->ItemFlags[7] % 30 == 0)
			{
				short laraDoubles[4];
				int i = 0;
				...
				{
					ItemInfo* item2 = &g_Level.Items[itemNum];
					short nextItem = item2->NextItem;
					if (item2->ObjectNumber == ID_LARA_DOUBLE && item2->TriggerFlags == 3 && item->RoomNumber == item2->RoomNumber)
					{
						laraDoubles[i++] = itemNum;
						if (i >= 4)
							break;
					}
					itemNum = nextItem;
				...
				if (i >= 4)
				{
					//0 - 3 = teleport to new lara double instead of attack
					//4 - 7 - continue attacking Lara
					int randInt = GetRandomControl() & (item->ItemFlags[7] >= 480 ? 3 : 7);
					if (randInt < 4 && laraDoubles[randInt] != item->ItemFlags[6])
					{
						creature->Enemy = &g_Level.Items[laraDoubles[randInt]];
						item->ItemFlags[6] = laraDoubles[randInt];
						item->ItemFlags[7] = 0;
					}
				}
			}
			...
			//wc - force attack when teleporting

			if (item->TriggerFlags == 999 && creature->Enemy != LaraItem)
