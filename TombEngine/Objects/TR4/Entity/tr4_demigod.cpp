...
namespace TEN::Entities::TR4
{
	...
	void TriggerDemigodMissile(ItemInfo* item, short roomNumber, int flags)
	{
		PHD_3DPOS src;
		...
		if (fxNumber != -1)
		{
			...
			if (flags < 4)
				...
				fx->pos.Orientation.y = src.Orientation.y + (GetRandomControl() & 0x7FF) - 1024;
			...
			fx->flag1 = flags;
			fx->speed = (GetRandomControl() & 0x1F) + 96 * 3;
	...
	void DoDemigodEffects(short itemNumber)
	{
		...
		if (animIndex == DEMIGOD2_ANIM_SINGLE_PROJECTILE_ATTACK)
		{
			if (item->Animation.FrameNumber == g_Level.Anims[item->Animation.AnimNumber].frameBase)
			{
				...
				if (item->ObjectNumber == ID_DEMIGOD3)
					TriggerDemigodMissile(item, item->RoomNumber, 3);
				else
					TriggerDemigodMissile(item, item->RoomNumber, 5);
			}
		}
		else if (animIndex == DEMIGOD3_ANIM_SINGLE_PROJECTILE_ATTACK)
		{
			if (item->Animation.FrameNumber <= g_Level.Anims[item->Animation.AnimNumber].frameBase + 3)
			{
				auto pos1 = Vector3Int(-544, 96, 0 );
				GetJointAbsPosition(item, &pos1, 16);
		...
		else if (animIndex == DEMIGOD3_ANIM_RADIAL_PROJECTILE_ATTACK)
		{
			...
			if (frameNumber >= 8 && frameNumber <= 64)
			{
				...
				auto angles = GetVectorAngles(pos2.x - pos1.x, pos2.y - pos1.y, pos2.z - pos1.z);
				auto pose = PHD_3DPOS(pos1, angles);
				TriggerDemigodMissile(item, item->RoomNumber, 4);
			}
		}
	}
