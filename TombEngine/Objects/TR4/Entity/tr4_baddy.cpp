...
namespace TEN::Entities::TR4
{
	...
	void InitialiseBaddy(short itemNumber)
	{
		...
		if (ocb > 100 && (item->ObjectNumber==ID_BADDY1 || ocb != 999) && ocb != 998 && ocb != 997)
		{
			item->Animation.AnimNumber = Objects[ObjectNumber].animIndex + BADDY_ANIM_CROUCH;
			item->Animation.FrameNumber = g_Level.Anims[item->Animation.AnimNumber].frameBase;
			item->Animation.TargetState = BADDY_STATE_CROUCH;
	...
	void BaddyControl(short itemNumber)
	{
		...
		if (item->TriggerFlags % 1000 && item->TriggerFlags != 999 && item->TriggerFlags != 998 && item->TriggerFlags != 997)
		{
			creature->MaxTurn = 0;
		...
		{
			//wc - used for freeze baddy time delay
			if (item->ItemFlags[7] > 0)
				item->ItemFlags[7]--;
			//wc both baddy1 and 2 use healthbar
			auto lara = GetLaraInfo(LaraItem);
			if(BaddyHealthItemNumber <= 0 && item == lara->TargetEntity)
				BaddyHealthItemNumber = itemNumber;
			//wc antigrav drop
			if (item->ObjectNumber == ID_BADDY2 && item->TriggerFlags == 997)
			{
				item->TriggerFlags = 10;
				creature->MaxTurn = 0;
			...
			//wc antigrav vault
			if (item->ObjectNumber == ID_BADDY2 && item->TriggerFlags == 998)
			{
				item->TriggerFlags = 10;
				creature->MaxTurn = 0;
				item->Animation.AnimNumber = Objects[ObjectNumber].animIndex + 72;
				item->Animation.ActiveState = 46;
				item->Animation.FrameNumber = g_Level.Anims[item->Animation.AnimNumber].frameBase;
				Weather.Flash(28, 242, 24, 0.06f);
				return;
			}
			...
			//wc antigrav jump
			if (item->ObjectNumber == ID_BADDY2 && item->TriggerFlags == 999)
			{
				item->TriggerFlags = 10;
				...
				currentCreature->LOT.IsJumping = true;
				item->Animation.TargetState = 45;
				Weather.Flash(28, 242, 24, 0.06f);
				return;
			}
			//wc boss puts fire out and gets burned by it
			if (item->ObjectNumber == ID_BADDY1)
			{
				int j = 0;
				for (int i = 0; i < g_Level.NumItems; i++)
				{
					...
					if (item2->ObjectNumber == ID_FLAME_EMITTER && item2->ItemFlags[7] > -1 && item->RoomNumber == item2->RoomNumber)
					{
						PHD_3DPOS* pos = &item2->Pose;
						if (ItemNearTarget(pos, item, CLICK(2)))
						{
							item->HitPoints -= 1;
							if (item->Animation.ActiveState != BADDY_STATE_PUT_FIRE_OUT)
								SetAnimation(item, BADDY_ANIM_PUT_FIRE_OUT);
							else if (item->Animation.FrameNumber >= g_Level.Anims[item->Animation.AnimNumber].frameBase + 47)
								item2->ItemFlags[7] = -1;
							break;
						}
					}
			...
			////wc - baddy2 should always be able to have Lara in range
			//if (item->ObjectNumber == ID_BADDY2)
			//{
			//	if (AI.distance >= pow(MAX_VISIBILITY_DISTANCE, 2))
			//		AI.distance = pow(MAX_VISIBILITY_DISTANCE, 2) - 2;
			//	AI.ahead = true;
			//}
			...
			//wc - prevent turning during crouch
			if(item->Animation.ActiveState != BADDY_STATE_CROUCH)
			...
			case BADDY_STATE_IDLE:
				...
				if (Targetable(item, &AI) && (item->ItemFlags[7] == 0 || item->ObjectNumber == ID_BADDY2))
				{
			...
			case BADDY_STATE_CROUCH:
				//wc - make sure freeze boss doesn't escape room
				if (item->ObjectNumber != ID_BADDY1 || item->RoomNumber == LaraItem->RoomNumber)
				{
					if (!item->ItemFlags[0])
					{
						if (currentCreature->Enemy)
			...
			case BADDY_STATE_FIRE:
				...
				//wc - force the freeze boss to move around a bit more
				if (!ShotLara(item, &AI, &BaddyGunBite, joint1, 15) || item->ItemFlags[7] > 127)
					item->Animation.TargetState = BADDY_STATE_IDLE;
				break;
		...
		if (item->Animation.ActiveState >= BADDY_STATE_JUMP_FORWARD_2_BLOCKS ||
			...
			item->Animation.ActiveState == BADDY_STATE_PUT_FIRE_OUT)
		{
