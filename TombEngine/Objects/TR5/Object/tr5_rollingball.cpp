...
void InitialiseRollingBall(short itemNum)
{
	...
	if (item->TriggerFlags == 1)
	{
		...
		for (int i = 0; i < g_Level.NumItems; i++)
		{
			...
			if (item2->ObjectNumber == ID_ANIMATING8)
			{
				Animatings[j++] = i;
			}
			if (j > 12)
...
void RollingBallCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (TestBoundsCollide(ballItem, laraItem, coll->Setup.Radius))
	{
		...
		{
			//wc - added stun check
			if (TriggerActive(ballItem) && ballItem->AfterDeath > -1 && (ballItem->ItemFlags[0] || ballItem->ItemFlags[1] || ballItem->Animation.VerticalVelocity))
			{
...
void RollingBallControl(short itemNumber)
{
	...
	//wc - custom rollingball puzzle
	if (item->TriggerFlags == 1)
	{
		short room = item->RoomNumber;
		item->Location.roomNumber = room;
		item->Location.yNumber = item->Pose.Position.y;
		int isOverFan = 0;
		//fan check
		for (int i = 0;i < 12;i++)
		{
			if (Animatings[i] == 0)
				continue;
			ItemInfo* fan = &g_Level.Items[Animatings[i]];
			if (TriggerActive(fan) && (item->Pose.Position.y - fan->Pose.Position.y > CLICK(10) * -1) && item->Pose.Position.y <= fan->Pose.Position.y && abs(item->Pose.Position.x - fan->Pose.Position.x) < CLICK(1) && abs(item->Pose.Position.z - fan->Pose.Position.z) < CLICK(1))
			{
				isOverFan = fan->TriggerFlags > 0 ? fan->TriggerFlags : 1;
				break;
			}
		}
		//floor check
		FloorInfo* floor = GetFloor(item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z, &room);
		room = item->RoomNumber;
		float slopeY = floor->FloorSlope(item->Pose.Position.x, item->Pose.Position.z).y;
		int height = GetFloorHeight(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z);
		int cheight = GetCeiling(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z);
		if(height <= item->Pose.Position.y + CLICK(2) && item->Animation.IsAirborne)
			SoundEffect(SFX_TR4_BOULDER_FALL, &item->Pose);
		item->Animation.IsAirborne = height > item->Pose.Position.y + CLICK(2);
		if (item->Animation.IsAirborne || (isOverFan > 0 && item->Animation.VerticalVelocity == 0))
		{
			item->Animation.VerticalVelocity += isOverFan > 0 ? GRAVITY * -1 * isOverFan : GRAVITY;
			if (abs(cheight - item->Pose.Position.y) < CLICK(2))
			{
				item->Pose.Position.y = cheight + CLICK(2);
		...
		{
			item->Animation.VerticalVelocity = 0;
			if(abs(item->Pose.Position.y - height) < CLICK(2))
				item->Pose.Position.y = height - CLICK(2);
		...
		item->Pose.Position.z += item->ItemFlags[1];
		UpdateItemRoom(item, 0);
		//rolling to the right
		if (item->ItemFlags[1] < 0)
		{
			item->Pose.Orientation.x += ANGLE(15);
			FloorInfo* floor = GetFloor(item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2), &room);
			int height = GetFloorHeight(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2));
			int cheight = GetCeiling(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2));
			if (floor->IsWall(item->Pose.Position.x, item->Pose.Position.z - CLICK(2)) || item->Pose.Position.y - height > CLICK(1) || cheight - item->Pose.Position.y > CLICK(1))
			{
				item->ItemFlags[1] = 0;
			}
		}
		...
		//stationary
		else if (item->ItemFlags[1] == 0 && (slopeY != 0 || item->ItemFlags[2] != 0))
		{
			FloorInfo* floor = GetFloor(item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2), &room);
			room = item->RoomNumber;
			int height = GetFloorHeight(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2));
			int cheight = GetCeiling(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z - CLICK(2));
			if ((slopeY < 0 || item->ItemFlags[2] == -1) && !floor->IsWall(item->Pose.Position.x, item->Pose.Position.z - CLICK(2)) && item->Pose.Position.y - height <= CLICK(1) && cheight - item->Pose.Position.y <= CLICK(1))
			{
				item->ItemFlags[1] = -2;
				item->ItemFlags[2] = -1;
			}
			...
			{
				floor = GetFloor(item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z + CLICK(2), &room);
				height = GetFloorHeight(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z + CLICK(2));
				cheight = GetCeiling(floor, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z + CLICK(2));
				if ((slopeY > 0 || item->ItemFlags[2] == 1) && !floor->IsWall(item->Pose.Position.x, item->Pose.Position.z + CLICK(2)) && item->Pose.Position.y - height <= CLICK(1) && cheight - item->Pose.Position.y <= CLICK(1))
				{
					item->ItemFlags[1] = 2;
					item->ItemFlags[2] = 1;
		...
		//sideways accel.
		if (item->ItemFlags[1] < 0 && item->ItemFlags[1] > -64)
			item->ItemFlags[1] -= 2;
		else if (item->ItemFlags[1] > 0 && item->ItemFlags[1]  < 64)
			item->ItemFlags[1] += 2;
		//prevent rotation from getting too large
		if(item->Pose.Orientation.x >= ANGLE(180) * -2)
			item->Pose.Orientation.x = 0;
		else if (item->Pose.Orientation.x <= ANGLE(180) * 2)
			item->Pose.Orientation.x = 0;
		//reset if it sits still long enough
		if (item->ItemFlags[1] == 0 && item->Animation.VerticalVelocity == 0)
			item->ItemFlags[3]++;
		else
			item->ItemFlags[3] = 0;
		...
		{
			item->Pose.Orientation.x = 0;
			item->Pose.Position.x = item->StartPose.Position.x;
			item->Pose.Position.y = item->StartPose.Position.y;
			item->Pose.Position.z = item->StartPose.Position.z;
			item->ItemFlags[0] = 0;
			item->ItemFlags[1] = 0;
			item->ItemFlags[2] = 0;
			item->ItemFlags[3] = 0;
			ItemNewRoom(itemNumber, item->ItemFlags[4]);
			RemoveActiveItem(itemNumber);
			Weather.Flash(255, 255, 255, 0.06f);
			SoundEffect(SFX_TR4_SAVE_CRYSTAL, &item->Pose);
		}
		return;
	}
