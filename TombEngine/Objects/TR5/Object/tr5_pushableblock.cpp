...
void PushableBlockControl(short itemNumber)
{
	...
	//wc - keep track of movement for sound control
	...
	//wc - handle pushblock sounds
	if (item->Pose.Position.x != oldX || item->Pose.Position.z != oldZ)
		DoPushPull = 1;
	else if (DoPushPull == 1)
		DoPushPull = -1;
}
...
bool TestBlockPush(ItemInfo* item, int blockHeight, unsigned short quadrant)
{
	...
	int i = 0;
	//wc - prevent lara double from getting in the way
	while (CollidedItems[i] != NULL && CollidedItems[i]->ObjectNumber != ID_LARA_DOUBLE && CollidedItems[i]->ObjectNumber != ID_LARA)
	{
...
bool TestBlockPull(ItemInfo* item, int blockHeight, short quadrant)
{
	...
	{
		//wc - prevent lara double and lara herself from stopping blocks
		if (CollidedItems[i] != item && CollidedItems[i]->ObjectNumber != ID_LARA_DOUBLE && CollidedItems[i]->ObjectNumber != ID_LARA) // if collided item is not pushblock in which lara embedded
		{
			if (Objects[CollidedItems[i]->ObjectNumber].floor == NULL)
				return false;
			else
