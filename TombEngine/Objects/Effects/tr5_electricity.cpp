...
void ElectricityWiresControl(short itemNumber)
{
	...
	while (CollidedItems[k] != nullptr)
	{
		...
		//wc - ignore lara double
		if (collItem->ObjectNumber == ID_LARA_DOUBLE)
			continue;
		...
		for (int i = 0; i < object->nmeshes; i++)
		{
			...
			//wc - I want this to be more of an obstacle
			if (TestBoundsCollide(item, collItem, CLICK(0.25f)))
			{
				CollisionInfo coll = {};
				coll.Setup.Radius = CLICK(0.25f); 
				coll.Setup.UpperCeilingBound = MAX_HEIGHT;
				coll.Setup.OldPosition = item->Pose.Position;
				coll.Setup.EnableObjectPush = true;
				instantKill = true;
				ItemPushItem(item, collItem, &coll, 0, 0);
			}
