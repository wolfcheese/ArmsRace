...
void TrapDoorControl(short itemNumber)
{
	...
	{
		if (!trapDoorItem->Animation.ActiveState && trapDoorItem->TriggerFlags >= 0 && trapDoorItem->TriggerFlags != 999)
			trapDoorItem->Animation.TargetState = 1;
	...
	if (trapDoorItem->Animation.ActiveState == 1)
	{
		if(trapDoorItem->ItemFlags[2] || JustLoaded)
			OpenTrapDoor(itemNumber);
		//wc - 999 is used for looping trapdoors
		if (trapDoorItem->TriggerFlags == 999 && TriggerActive(trapDoorItem))
		{
			trapDoorItem->ItemFlags[7] = 0;
			trapDoorItem->Animation.TargetState = 0;
		}
	}
	else if (!trapDoorItem->Animation.ActiveState)
	{
		if(!trapDoorItem->ItemFlags[2])
			CloseTrapDoor(itemNumber);
		//wc - 999 is used for looping trapdoors
		if (trapDoorItem->TriggerFlags == 999 && TriggerActive(trapDoorItem))
		{
			trapDoorItem->ItemFlags[7]++;
			if (trapDoorItem->ItemFlags[7] >= 20)
			{
				trapDoorItem->ItemFlags[7] = 0;
				trapDoorItem->Animation.TargetState = 1;
			}
