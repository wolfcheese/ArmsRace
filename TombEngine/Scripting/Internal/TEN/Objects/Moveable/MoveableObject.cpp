...
// @tparam int index the index of the desired anim 
	ScriptReserved_SetAnimNumber, &Moveable::SetAnimNumber,
		...
		ScriptReserved_SetWeapon, &Moveable::SetWeapon,
...
// @tparam int frame the new frame number
	ScriptReserved_SetFrameNumber, &Moveable::SetFrameNumber,
		...
		"SetTargetState", &Moveable::SetTargetState,
...
// @tparam int HP the amount of HP to give the moveable
	ScriptReserved_SetHP, &Moveable::SetHP,
			...
			/// Get Item Flag at index
// @function Moveable:GetItemFlagAt
ScriptReserved_GetItemFlagAt, & Moveable::GetItemFlagAt,

/// Set Item Flag at index
// @function Moveable:SetItemFlagAt
		ScriptReserved_SetItemFlagAt, &Moveable::SetItemFlagAt,
...
// @treturn bool true if the moveable was hit by something in the last gameplay frame, false otherwise 
	ScriptReserved_GetHitStatus, &Moveable::GetHitStatus,
		...
		ScriptReserved_SetHitStatus, &Moveable::SetHitStatus,
...
void Moveable::SetTimer(int value)
{
	m_item->Timer = value;
}
...
short Moveable::GetItemFlagAt(short index)
{
	return m_item->ItemFlags[index - 1];
}

void Moveable::SetItemFlagAt(short value, short index)
{
	m_item->ItemFlags[index-1] = value;
}
...
void Moveable::SetWeapon(int number)
{
	auto lara = GetLaraInfo(m_item);
	LaraInfo*& laraInfo = m_item->Data;
	Lara.Control.Weapon.GunType = Lara.Control.Weapon.RequestGunType = Lara.Control.Weapon.LastGunType = static_cast<LaraWeaponType>(number);
	Lara.Control.HandStatus = HandStatus::Free;
	InitialiseNewWeapon(m_item);
	lara->MeshPtrs[LM_LHAND] = Objects[ID_LARA_SKIN].meshIndex + LM_LHAND;
	lara->MeshPtrs[LM_RHAND] = Objects[ID_LARA_SKIN].meshIndex + LM_RHAND;
	if (number == 1)
	{
		lara->MeshPtrs[LM_LHAND] = Objects[WeaponObjectMesh(LaraItem, static_cast<LaraWeaponType>(number))].meshIndex + LM_LHAND;
...
void Moveable::SetTargetState(int state)
{
...
void Moveable::SetHitStatus(bool value)
{
	m_item->HitStatus = value;
}
