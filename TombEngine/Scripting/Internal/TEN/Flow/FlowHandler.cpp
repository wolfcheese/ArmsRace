...
void FlowHandler::SetIntroImagePath(std::string const& path, std::string const& path2)
{
	if(g_Configuration.language == 0)
		IntroImagePath = path;
	else
		IntroImagePath = path2;
}

void FlowHandler::SetTitleScreenImagePath(std::string const& path, std::string const& path2)
{
	if (g_Configuration.language == 0)
		TitleScreenImagePath = path;
	else
		TitleScreenImagePath = path2;
}
...
{
	return false; //wc - not sure how this is supposed to be configured elsewhere?
}
