...
// @mem rot
		"rot", sol::property(&GameScriptItemInfo::GetRot, &GameScriptItemInfo::SetRot),
			"weapon", sol::property(&GameScriptItemInfo::GetRot, &GameScriptItemInfo::SetWeapon),
...
void GameScriptItemInfo::SetWeapon(int number)
{
	LaraInfo*& laraInfo = m_item->data;
	laraInfo->gunType = laraInfo->requestGunType = laraInfo->lastGunType = static_cast<LARA_WEAPON_TYPE>(number);
	...
	if (number == 1)
	{
		Lara.meshPtrs[LM_LHAND] = Objects[WeaponObjectMesh(LaraItem, number)].meshIndex + LM_LHAND;
	}
}
