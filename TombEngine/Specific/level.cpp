...
void LoadObjects()
{
	...
	for (int i = 0; i < numStatics; i++)
	{
		...
		//wc - hardcoding shattertype as none. I don't need shatterable statics except for static #50
		StaticObjects[meshID].shatterType = meshID == 50 ? SHT_FRAGMENT : SHT_NONE;
		ReadInt16();
...
void ReadRooms()
{
	...
	for (int i = 0; i < numRooms; i++)
	{
		...
		for (int j = 0; j < numLights; j++)
		{
			...
			light.type = ReadInt8();
			light.castShadows = light.intensity >= 0.5f; //wc - my lazy way to enable shadows for all of the brighter lights
			ReadInt8(); 
...
unsigned int _stdcall LoadLevel(void* data)
{
	...
	std::mbstowcs(loadscreenFileName, g_Configuration.language == 0 ? level->LoadScreenFileName.c_str() : level->LoadScreenFileNameFR.c_str(), 80);
