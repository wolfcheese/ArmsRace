...
bool SaveConfiguration()
{
	...
	if (SetDWORDRegKey(rootKey, "Language", g_Configuration.language) != ERROR_SUCCESS) {
		RegCloseKey(rootKey);
		return false;
	}

	if (SetDWORDRegKey(rootKey, "FastLoad", g_Configuration.fastLoad) != ERROR_SUCCESS) {
		RegCloseKey(rootKey);
		return false;
	}
...
bool LoadConfiguration()
{
	...
	DWORD language = 0;
	if (GetDWORDRegKey(rootKey, "Language", &language, 1) != ERROR_SUCCESS)
	{
		RegCloseKey(rootKey);
		return false;
	}
	...
	DWORD fastLoad = 0;
	if (GetDWORDRegKey(rootKey, "FastLoad", &fastLoad, 1) != ERROR_SUCCESS)
	{
		RegCloseKey(rootKey);
		return false;
	}
	...
	g_Configuration.language = language;
	g_Configuration.fastLoad = fastLoad;
