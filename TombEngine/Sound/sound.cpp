		...
		try {
			...
			for (const auto& file : std::filesystem::directory_iterator{ dir })
			{
				...
				if (!result.empty())
				{
					...
					//wc - these are my looping tracks. Kind of annoying that the support for Tracks.lua was dropped.
					if (index == 107 || index == 112 || index == 102 || index == 111)
					{
				...
				//wc final boss music
				else if (fileName.find("Free (Orchestral Mix)") != std::string::npos)
				{
					SoundTrackInfo s;
					s.Name = "Free (Orchestral Mix)";
					SoundTracks.insert(std::make_pair(113, s));
				}
			}
		}
