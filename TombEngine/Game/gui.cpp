...
InventoryObject inventry_objects_list[INVENTORY_TABLE_SIZE] =
{
	...
	{ID_CROSSBOW_ITEM, 0, 0.5f, ANGLE(-90), ANGLE(33), 0, OPT_EQUIP | OPT_CHOOSEAMMO_CROSSBOW, STRING_CROSSBOW, 1, INV_ROT_Y},
	...
	{ID_PC_LOAD_INV_ITEM, -1, 0.1f, ANGLE(180), 0, 0, OPT_LOAD, STRING_LOAD_GAME, NO_JOINT_BITS, INV_ROT_Y},
	{ID_PC_LOAD_SAVE_ITEM, -1, 0.1f, ANGLE(180), 0, 0, OPT_SAVE, STRING_SAVE_GAME, NO_JOINT_BITS, INV_ROT_Y},
	...
	{ID_COMPASS_ITEM, -14, 1.0f, 0, 0, 0, 0, STRING_LOAD_GAME, NO_JOINT_BITS, INV_ROT_Y},
...
InventoryResult GuiController::TitleOptions()
{
	...
	case Menu::Language:
		option_count = 2;
		HandleLanguageSettingsInput(false);
		break;
	}
...
void GuiController::HandleOtherSettingsInput(bool pause)
{
	...
	if (goLeft || goRight)
	{
		...
		case 6:
			SoundEffect(SFX_TR4_MENU_CHOOSE, nullptr, SoundEnvironment::Always);
			CurrentSettings.conf.fastLoad = CurrentSettings.conf.fastLoad > 0 ? 0 : 1;
			break;
		}
	}
...
//wc - language select
void GuiController::HandleLanguageSettingsInput(bool pause)
{
	UpdateInput();
	DoDebouncedInput();
	...
	if (goLeft)
	{
		...
		{
		case 0:
			SoundEffect(SFX_TR4_MENU_CHOOSE, NULL, SoundEnvironment::Always);
			CurrentSettings.conf.language = CurrentSettings.conf.language == 0 ? 1 : 0;
			break;
		}
	}
	...
	if (goUp)
	{
		...
		SoundEffect(SFX_TR4_MENU_CHOOSE, 0, SoundEnvironment::Always);
	}
	...
	if (goSelect)
	{
		SoundEffect(SFX_TR4_MENU_SELECT, NULL, SoundEnvironment::Always);
		...
		if (selected_option == 1)
		{
			...
			g_Renderer.UpdateLogo(logoName);

			menu_to_display = pause ? Menu::Pause : Menu::Options;
			selected_option = pause ? 2 : 3;
		}
...
InventoryResult GuiController::DoPauseMenu()
{
	...
	case Menu::Language:
		option_count = 2;
		HandleLanguageSettingsInput(true);
		break;
	}
	...
	if (goDeselect)
	{
		...
		//wc - exit from statistics when level is finished
		if (menu_to_display == Menu::Statistics && LevelComplete > CurrentLevel)
		{
			SoundEffect(SFX_TR4_MENU_SELECT, nullptr, SoundEnvironment::Always);
...
InventoryResult GuiController::DoGameOverMenu()
{
	ClearInputVariables(1);
	UpdateInput();
	DoDebouncedInput();
	...
	//wc - using hacky approach to get "key up" of select input
	if (goSelect)
		option_count = 999;

	if (!goSelect && !dbSelect && option_count == 999)
	{
		option_count = 1;
		switch (selected_option)
		{
		case 0:
			...
			else
			{
				...
				selected_option = 0;
				return InventoryResult::LoadGame;
			}
			break;
		case 1:
			invMode = InventoryMode::None;
...
{
	//wc - bandaid fix for weird pistol combine glitch
	if (obj == INV_OBJECT_PISTOLS)
		return 0;
...
void GuiController::DoInventory()
{
	...
	else
	{
		...
		if (!ammo_active)
		{
			...
			if ((opts & (OPT_CHOOSEAMMO_SHOTGUN | OPT_CHOOSEAMMO_GRENADEGUN)))
			{
...
void GuiController::DrawAmmoSelector()
{
	...
	if (num_ammo_slots > 0)
	{
		for (n = 0; n < num_ammo_slots; n++)
		{
			...
			//wc - hack to handle hardcoded ammo3 in crossbow
			if (n == *current_ammo_type || *current_ammo_type >= num_ammo_slots)
			{
			...
			if (n == *current_ammo_type || *current_ammo_type >= num_ammo_slots)
			{
				...
				{
					if(g_Configuration.language == 0)
						sprintf(&invTextBuffer[0], "%s %s", g_GameFlow->GetString("unlimited_ammo"), g_GameFlow->GetString(inventry_objects_list[ammo_object_list[n].invitem].objname));
					else
						sprintf(&invTextBuffer[0], "%s %s", g_GameFlow->GetString(inventry_objects_list[ammo_object_list[n].invitem].objname), g_GameFlow->GetString("unlimited_ammo"));
				}
				else
			...
			if (!i)
			{
					...
					inventry_objects_list[rings[ringnum]->current_object_list[n].invitem].object_number > ID_PUZZLE_ITEM16)
				{
					if (nummeup)
					{
						...
						{
							if (g_Configuration.language == 0)
								sprintf(textbufme, "%s %s", g_GameFlow->GetString("unlimited_ammo"), g_GameFlow->GetString(inventry_objects_list[rings[ringnum]->current_object_list[n].invitem].objname));
							else
								sprintf(textbufme, "%s %s", g_GameFlow->GetString(inventry_objects_list[rings[ringnum]->current_object_list[n].invitem].objname), g_GameFlow->GetString("unlimited_ammo"));
						}
						else
							sprintf(textbufme, "%d x %s", nummeup, g_GameFlow->GetString(inventry_objects_list[rings[ringnum]->current_object_list[n].invitem].objname));
					}
...
{
	g_Renderer.DrawObjectOn2DPosition(130, 380, ID_COMPASS_ITEM, ANGLE(90), 0, ANGLE(180), inventry_objects_list[INV_OBJECT_COMPASS].scale1);
