...
void LaserSightCamera(ItemInfo* item)
{
	...
	if (!(RawInput & IN_ACTION) || WeaponDelay || (!ammo && !restoreAmmo))
	{
	...
	else
	{
		if (lara->Control.Weapon.GunType == LaraWeaponType::Revolver)
		{
			...
			if (!ammo.hasInfinite() && !restoreAmmo)
				(ammo)--;
...
void BinocularCamera(ItemInfo* item)
{
	...
	int l = SECTOR(40.5f) * phd_cos(headXRot);
...
void CalculateCamera()
{
	...
	if ((Camera.type != CameraType::Heavy && Camera.type != CameraType::Fixed ||
		Camera.timer == -1))
	{
		Camera.type = CameraType::Chase;
		Camera.speed = 10;
...
void LookUpDown(ItemInfo* item)
{
	...
	{
		TrInput &= ~IN_FORWARD;
		if (lara->ExtraHeadRot.x > -ANGLE(65.0f))
		{
			if (BinocularRange)
	...
	{
		TrInput &= ~IN_BACK;
		if (lara->ExtraHeadRot.x < ANGLE(60.0f))
		{
			if (BinocularRange)
