...
int TriggerActive(ItemInfo* item)
{
	...
	else
	{
		if (item->Timer)
		{
			if (item->Timer > 0)
			{
				//wc - when a timed object is used (and is not the switch itself), display the remaining time
				if (item->ObjectNumber < ID_SWITCH_TYPE1 || item->ObjectNumber > ID_SWITCH_TYPE16)
				{
					std::string tmp = std::to_string(item->Timer / 30);
					const char* chars = tmp.c_str();
					g_Renderer.AddString(400, 32, chars, PRINTSTRING_COLOR_WHITE, PRINTSTRING_CENTER);
				}
				--item->Timer;
				if (!item->Timer)
					item->Timer = -1;
...
int SwitchTrigger(short itemNumber, short timer)
{
	...
	//wc - treat twoblock platform as switch
	if (item->ObjectNumber == ID_TWOBLOCK_PLATFORM && (item->ItemFlags[7] == 0 || (item->Pose.Position.y >= item->ItemFlags[0] + 12 && item->Pose.Position.y <= item->ItemFlags[0] + 15)))
	{
		item->ItemFlags[7] = 1;
		return 1;
	}
...
void TestTriggers(FloorInfo* floor, int x, int y, int z, bool heavy, int heavyFlags)
{
	...
	else
	{
		...
		case TRIGGER_TYPES::SWITCH:
			...
			if (ObjectNumber == ID_TWOBLOCK_PLATFORM && g_Level.Items[value].ItemFlags[1] < 0)
				switchFlag = 1;
