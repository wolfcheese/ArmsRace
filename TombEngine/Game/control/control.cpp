...
GameStatus ControlPhase(int numFrames, int demoMode)
{
	...
	for (framesCount += numFrames; framesCount > 0; framesCount -= 2)
	{
		...
		while (g_Gui.GetInventoryMode() == InventoryMode::GameOver)
		{
			g_Gui.DrawInventory();
			g_Renderer.Synchronize();
			//wc - user exits from game over screen
			switch (g_Gui.DoGameOverMenu())
			{
				case InventoryResult::ExitToTitle:
					return GameStatus::ExitToTitle;
					break;
		...
		while (CurrentLevel != 0 && LevelComplete)
		{
			//wc - show stats at end of level and play music
			if (LevelComplete == CurrentLevel + 1)
			{
				LevelComplete++;
				g_Gui.SetInventoryMode(InventoryMode::Statistics);
				g_Gui.menu_to_display = Menu::Statistics;
				PlaySoundTrack("001", SoundTrackType::OneShot);
			}
			//wc - user exits from stats screen
			g_Gui.DrawInventory();
			g_Renderer.Synchronize();
			if (g_Gui.DoPauseMenu() == InventoryResult::ExitToTitle)
				return GameStatus::ExitToTitle;
		...
		// Is Lara dead?
		if (CurrentLevel != 0 && Lara.Control.Count.Death > 60)
		{
			StopAllSounds();

			//wc - changing this to show a game over menu
			g_Gui.SetInventoryMode(InventoryMode::GameOver);
		}
		...
		if (CurrentLevel != 0)
			HandleOptics();
					...
					//Lara.TargetEntity = nullptr; wc - is this supposed to be here? It's breaking a lot of things
		...
		while (itemNumber != NO_ITEM)
		{
			...
			{
				//wc - AfterDeath for shotgun stun
				if (item->AfterDeath < 0)
				{
					item->AfterDeath++;
					itemNumber = nextItem;
					continue;
		...
		if (CurrentLevel != 0)
		{
			...
			// Control Lara
			InItemControlLoop = true;
			//wc - don't update Lara if she is frozen
			if (LaraItem->AfterDeath < 0)
				LaraItem->AfterDeath++;
			else
...
GameStatus DoTitle(int index, std::string const& ambient)
{
	...
	if (g_GameFlow->TitleType == TITLE_TYPE::FLYBY)
	{
		...
		PlaySoundTrack(ambient, SoundTrackType::OneShot);
...
GameStatus DoLevel(int index, std::string const& ambient, bool loadFromSavegame)
{
	...
	else
	{
		...
		// wc - not when loaded from a savegame
	...
	while (DoTheGame)
	{
			...
			result == GameStatus::LevelComplete)
		{
			...
			//wc - fast load that doesn't reload the entire level - need to test a full playthrough!
			if (result == GameStatus::LoadGame && g_Configuration.fastLoad)
			{
				...
				DisableBubbles();
				DisableDebris();
				//wc - reset custom timers
				AntiGravTimer = 0;
				FreezeTimer = 0;
				BaddyHealthItemNumber = -1;

				SaveGameHeader header;
				SaveGame::LoadHeader(g_GameFlow->SelectedSaveGame, &header);
				...
				Weather.Flash(255, 255, 255, 0.1f);
			}
			else
				return result;
		}
