...
void lara_col_hang(ItemInfo* item, CollisionInfo* coll)
{
		...
		item->Animation.AnimNumber == LA_HANG_IDLE)
	{
				...
				item->Animation.AnimNumber == LA_HANG_IDLE))
		{
			...
			{
				//wc - flimsy overhead ladder check to monkey
				//incl. bonus hack to prevent Lara from swinging through flipped rooms ("iced" walls)
				int dx = int(sin(TO_RAD(item->Pose.Orientation.y)) * CLICK(1));
				int dz = int(cos(TO_RAD(item->Pose.Orientation.y)) * CLICK(1));
				short troomnumber = item->RoomNumber;
				auto monkeyColl = GetCollision(item->Pose.Position.x + dx, item->Pose.Position.y + CLICK(1), item->Pose.Position.z + dz, item->RoomNumber);
				int ceilingHeight = monkeyColl.Block->CeilingHeight(item->Pose.Position.x + dx, item->Pose.Position.z + dz);
				if ((g_Level.Rooms[item->RoomNumber].flipNumber < 0 || FlipStats[g_Level.Rooms[item->RoomNumber].flipNumber] != 1) && 
					monkeyColl.BottomBlock->Flags.Monkeyswing &&
					ceilingHeight <= item->Pose.Position.y - CLICK(1))
				{
					...
					if (item->Animation.AnimNumber != 566)
					{
						item->Animation.AnimNumber = 566;
