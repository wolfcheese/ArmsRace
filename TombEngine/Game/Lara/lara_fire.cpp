...
bool WillRestoreAmmo()
{
	bool restoreAmmo = false;
	auto* lara = GetLaraInfo(LaraItem);
	if (lara->TargetEntity)
	{
		if (lara->TargetEntity->Data.is<CreatureInfo>())
		{
			if (Objects[lara->TargetEntity->ObjectNumber].undead)
				restoreAmmo = true;
			else if (lara->TargetEntity->ObjectNumber == ID_LARA_DOUBLE)
				restoreAmmo = true;
		}
		else
			restoreAmmo = true;
	...
	else if(LaserSight)
	{
		MESH_INFO* mesh;
		Vector3Int vector;
		int itemNumber = ObjectOnLOS2(&Camera.pos, &Camera.target, &vector, &mesh);
		if (itemNumber != NO_LOS_ITEM && itemNumber > 0)
		{
			...
			if (item->Data.is<CreatureInfo>())
			{
				if (Objects[item->ObjectNumber].undead)
					restoreAmmo = true;
				else if (item->ObjectNumber == ID_LARA_DOUBLE)
					restoreAmmo = true;
			}
		}
	}
...
void LaraGun(ItemInfo* laraItem)
{
	...
	else if (lara->Control.HandStatus == HandStatus::Free)
	{
		...
		if (TrInput & IN_DRAW || KeyMap[KC_6] || KeyMap[KC_5] || KeyMap[KC_4] || KeyMap[KC_3] ||
			KeyMap[KC_2] || KeyMap[KC_1] || lara->Control.Weapon.RequestGunType != lara->Control.Weapon.GunType)
		{
	...
	case HandStatus::WeaponReady:
		...
		if (TrInput & IN_ACTION)
		{
			...
			bool restoreAmmo = WillRestoreAmmo();
			if (!GetAmmo(laraItem,lara->Control.Weapon.GunType) && !restoreAmmo)
			{
...
{	
	auto* lara = GetLaraInfo(laraItem);
	//wc - failsafe for non-creatures
	if (!target->Data.is<CreatureInfo>())
		return;
	//wc - failsafe for lara double to not die
...
FireWeaponType FireWeapon(LaraWeaponType weaponType, ItemInfo* target, ItemInfo* src, Vector3Shrt armOrient, bool adjustAmmo)
{
	auto* lara = GetLaraInfo(src);

	//wc - give player ammo if it's to use gun effect: targeted immortal enemy or non-creature or Lara_Double
	bool restoreAmmo = WillRestoreAmmo();
	auto& ammo = GetAmmo(src, weaponType);
	if (restoreAmmo && !ammo.hasInfinite() && adjustAmmo)
		ammo++;
	if (ammo.getCount() == 0 && !ammo.hasInfinite())
		return FireWeaponType::NoAmmo;
	if (!ammo.hasInfinite() && adjustAmmo)
		ammo--;
	...
	auto direction = DirectX::SimpleMath::Vector3(x, y, z);
	direction.Normalize();

	auto source = DirectX::SimpleMath::Vector3(pos.x, pos.y, pos.z);
	...
	for (int i = 0; i < num; i++)
	{
		auto sphere = BoundingSphere(DirectX::SimpleMath::Vector3(CreatureSpheres[i].x, CreatureSpheres[i].y, CreatureSpheres[i].z), CreatureSpheres[i].r);
		float distance;
	...
	else
	{
		...
		//wc - shotgun stop effect for enemies
		if (weaponType == LaraWeaponType::Shotgun && FreezeTimer <= 0 && target->ObjectNumber != ID_HORSEMAN)
		{
			Weather.Flash(2,37,179, 0.06f);
			target->AfterDeath = -128;
			FreezeTimer = 127;
		}
		//wc - uzis disable fires
		else if ((lara->Control.Weapon.GunType == LaraWeaponType::Uzi && target->ItemFlags[7] >= 0 && (target->ObjectNumber == ID_FLAME_EMITTER || target->ObjectNumber == ID_FLAME_EMITTER2)))
		{
			Weather.Flash(7, 215, 247, 0.06f);
			target->ItemFlags[7] = -1;
		}
		//}
...
void FindTargetPoint(ItemInfo* item, GameVector* target)
{
	...
	//wc - super hacky, but trapdoor and expanding platforms are bridges and can't pass LOS check by default
	if (item->ObjectNumber == ID_TRAPDOOR2 || item->ObjectNumber == ID_EXPANDING_PLATFORM)
		target->y -= CLICK(1);
}
...
void LaraGetNewTarget(ItemInfo* laraItem, WeaponInfo* weaponInfo)
{
	...
	for (int slot = 0; slot < ActiveCreatures.size(); ++slot)
	{
		if (ActiveCreatures[slot]->ItemNumber != NO_ITEM)
		{
			...
			if (item->HitPoints > 0)
			{
				//wc - lara_double should only be targeted by relevant weapon
				if (item->ObjectNumber == ID_LARA_DOUBLE)
				{
					...
					case 3:
						...
						break;
					case 4:
						if (lara->Control.Weapon.GunType != LaraWeaponType::Uzi && lara->Control.Weapon.GunType != LaraWeaponType::Crossbow)
							continue;
						break;
					case 5:
	...
	//wc - for shotgun, scan for traps. For uzis and crossbow, scan for flames
	if (lara->Control.Weapon.GunType == LaraWeaponType::Shotgun || lara->Control.Weapon.GunType == LaraWeaponType::Uzi || lara->Control.Weapon.GunType == LaraWeaponType::Crossbow)
	{
		...
		while (itemNum != NO_ITEM)
		{
			...
			if ((lara->Control.Weapon.GunType == LaraWeaponType::Shotgun && ((item->ObjectNumber == ID_EXPANDING_PLATFORM) || (item->ObjectNumber == ID_COG) || (item->ObjectNumber == ID_PLINTH_BLADE) || (item->ObjectNumber == ID_TRAPDOOR2 && item->ItemFlags[2]) || (item->ObjectNumber == ID_ROLLINGBALL && (item->ItemFlags[0] || item->Animation.VerticalVelocity))))
				|| (lara->Control.Weapon.GunType == LaraWeaponType::Uzi && item->ItemFlags[7] >= 0 && (item->ObjectNumber == ID_FLAME_EMITTER || item->ObjectNumber == ID_FLAME_EMITTER2))
				|| (lara->Control.Weapon.GunType == LaraWeaponType::Crossbow && item->ItemFlags[7] < 0 && (item->ObjectNumber == ID_FLAME_EMITTER || item->ObjectNumber == ID_FLAME_EMITTER2 || item->ObjectNumber == ID_FIREROPE)))
			{
				int x = item->Pose.Position.x - src.x;
				int y = item->Pose.Position.y - src.y;
				int z = item->Pose.Position.z - src.z;
