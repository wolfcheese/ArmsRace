...
void lara_as_monkey_idle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_ACTION && lara->Control.CanMonkeySwing)
	{
		...
		//wc - flimsy overhead ladder check from monkey
		//incl. bonus hack to prevent Lara from swinging through flipped rooms ("iced" walls)
		else if (TrInput & IN_FORWARD && (g_Level.Rooms[item->RoomNumber].flipNumber < 0 || FlipStats[g_Level.Rooms[item->RoomNumber].flipNumber] != 1))
		{
			int dx = int(sin(TO_RAD(item->Pose.Orientation.y)) * CLICK(2));
			int dz = int(cos(TO_RAD(item->Pose.Orientation.y)) * CLICK(2));
			short troomnumber = item->RoomNumber;
			if ((1 << (GetQuadrant(item->Pose.Orientation.y + ANGLE(180)) + 8)) & GetClimbFlags(GetFloor(item->Pose.Position.x + dx, item->Pose.Position.y, item->Pose.Position.z + dz, &troomnumber)))
			{
				SnapItemToLedge(item, coll, item->Pose.Orientation.y);
				item->Animation.TargetState = LS_KICK;
				return;
			}
		}
