...
void FireShotgun(ItemInfo* laraItem)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (FireWeapon(LaraWeaponType::Shotgun, lara->TargetEntity, laraItem, wobbleArmOrient, i == 0) != FireWeaponType::NoAmmo)
			fired = true;
	}
...
void DrawShotgun(ItemInfo* laraItem, LaraWeaponType weaponType)
{
	...
	{
		//wc - check for existing weaponitem
		for (int i = 0; i < g_Level.NumItems; i++)
		{
			item = &g_Level.Items[i];
			if (item->ObjectNumber == WeaponObject(weaponType))
			{
				lara->Control.Weapon.WeaponItem = i;
				break;
			}
		}
		if (lara->Control.Weapon.WeaponItem == NO_ITEM)
...
void FireRocket(ItemInfo* laraItem)
{
	...
	if (restoreAmmo && !ammos.hasInfinite())
		ammos++;
	if (!ammos)
		return;
...
void RocketControl(short itemNumber)
{
	...
	for (int n = 0; n < 2; n++)
	{
		...
		//wc - rockets exploding too early with original logic
		radius = (explode ? ROCKET_EXPLODE_RADIUS : ROCKET_HIT_RADIUS);
	...
	for (int n = 0; n < 2; n++)
	{
		...
		if (CollidedItems[0])
		{
			...
			do
			{
				...
				//wc - ignite flame
				if (explode && currentItem->ItemFlags[7] < 0 && (currentItem->ObjectNumber == ID_FLAME_EMITTER || currentItem->ObjectNumber == ID_FLAME_EMITTER2))
				{
					Weather.Flash(242, 161, 22, 0.06f);
					currentItem->ItemFlags[7] = 0;
					continue;
				}
				
				//wc - ignite firerope
				if (explode && currentItem->ObjectNumber == ID_FIREROPE && currentItem->ItemFlags[7] <= 0)
				{
					currentItem->ItemFlags[7] = 1;
					TestTriggers(currentItem, true);
					for (int i = 0;i < 14;i++)
					{
						ExplodeItemNode(currentItem, i, 0, BODY_EXPLODE);
					}
					continue;
				}
				if ((currentObj->intelligent && currentObj->collision && currentItem->Status == ITEM_ACTIVE && !currentObj->undead) ||
