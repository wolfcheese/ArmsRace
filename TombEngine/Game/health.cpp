...
int FreezeTimer = 0;
int AntiGravTimer = 0;
short BaddyHealthItemNumber = -1;
extern RendererHUDBar* g_HealthBar;
extern RendererHUDBar* g_DashBar;
extern RendererHUDBar* g_AirBar;
extern RendererHUDBar* g_FreezeBar;
extern RendererHUDBar* g_AntiGravBar;
extern RendererHUDBar* g_BaddyBar;
...
void DrawHUD(ItemInfo* item)
{
	...
	{
		AntiGravTimer--;
		g_Renderer.DrawBar(AntiGravTimer / 800.0f, g_AntiGravBar, ID_SFX_BAR_TEXTURE, 0, 0);
	}
	if (FreezeTimer > 0)
	{
		FreezeTimer--;
		g_Renderer.DrawBar(FreezeTimer / 120.0f, g_FreezeBar, ID_AIR_BAR_TEXTURE, 0, 0);
	}
	if (BaddyHealthItemNumber > 0)
	{
		auto item = &g_Level.Items[BaddyHealthItemNumber];
		if (item->HitPoints > 0)
			g_Renderer.DrawBar(item->HitPoints / (float)Objects[item->ObjectNumber].HitPoints, g_BaddyBar, ID_HEALTH_BAR_TEXTURE, 0, 0);
		else
