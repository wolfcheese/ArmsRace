...
void LoadSavegameInfos()
{
	...
	for (int i = 0; i < SAVEGAME_MAX; i++)
	{
		...
		//wc - make savegame count global
		if (SavegameInfos[i].Count > maxCount)
			maxCount = SavegameInfos[i].Count;
	}
	SaveGame::LastSaveGame = maxCount;
}
...
bool SaveGame::Save(int slot)
{
	...
	sghb.add_days(days);
	sghb.add_hours(hours);
	sghb.add_minutes(min);
	sghb.add_seconds(sec);
	...
	tightropeControl.add_tightrope_item(FreezeTimer);
	tightropeControl.add_time_on_tightrope(AntiGravTimer);
...
bool SaveGame::Load(int slot)
{
	...
	{
		//wc - update flipmaps for fast reload
		if (s->flip_stats()->Get(i) != 0 && FlipStats[i] != 1)
			DoFlipMap(i);
		else if(s->flip_stats()->Get(i) == 0 && FlipStats[i] != 0)
			DoFlipMap(i);
	...
	for (int i = 0; i < s->items()->size(); i++)
	{
		...
		item->AfterDeath = savedItem->after_death();
	...
	//wc - super duper hacky, using tightrope values to save custom timers
	FreezeTimer = s->lara()->control()->tightrope()->tightrope_item();
	AntiGravTimer = s->lara()->control()->tightrope()->time_on_tightrope();
