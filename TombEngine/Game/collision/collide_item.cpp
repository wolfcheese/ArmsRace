	...
	for (auto i : g_Level.Rooms[collidingItem->RoomNumber].neighbors)
	{
		...
		if (collidedItems)
		{
			...
			if (itemNumber != NO_ITEM)
			{
				...
				{
					auto* item = &g_Level.Items[itemNumber];
					//wc - make sure lara_double doesn't get too annoyingly in the way
					auto lara = GetLaraInfo(LaraItem);
					if (lara->TargetEntity != nullptr && (collidingItem->ObjectNumber == ID_ROCKET || collidingItem->ObjectNumber == ID_CROSSBOW_BOLT) && item->ObjectNumber == ID_LARA_DOUBLE && !LaserSight && lara->TargetEntity->LuaName != item->LuaName)
					{
						itemNumber = item->NextItem;
						continue;
					}
					if (collidingItem->ObjectNumber != ID_CROSSBOW_BOLT || (collidingItem->ObjectNumber == ID_CROSSBOW_BOLT && item->ObjectNumber != ID_FLAME_EMITTER && item->ObjectNumber != ID_FIREROPE && item->ObjectNumber != ID_FLAME_EMITTER2))
					{
...
void CreatureCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (waterPlayerCollision || coll->Setup.EnableObjectPush)
	{
					...
					{
						item->TriggerFlags = 3;
						SoundEffect(SFX_TR4_THUNDER_CRACK, NULL, SoundEnvironment::Always);
